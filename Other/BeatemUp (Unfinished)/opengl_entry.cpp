...
void opengl_init(int width, int height, char* name)
{
	...
	defaultShader = create_shader("shaders/def_vert.vs", "shaders/def_frag.fs");
}

static unsigned int create_shader(const char* vertex_path, const char* fragment_path)
{
	int result;
	int  success;
	char infoLog[512];
	...
	fp = fopen(vertex_path, "r");
	{
		...
		vertex_source[i] = '\0';
		const char* const_source = vertex_source;
		glShaderSource(vertex_shader, 1, &const_source, 0);
	...
	fp = fopen(fragment_path, "r");
	{
		...
		fragment_source[i] = '\0';
		const char* const_source = fragment_source;
		glShaderSource(fragment_shader, 1, &const_source, 0);
